...
steps/align_si.sh --nj $nj --cmd "run.pl" $datadir $langdir $traindir/tri5a $targetdir/tri5a_ali
...
for i in $(seq 1 $nj); do
  ...
  gunzip -c $targetdir/tri5a_ali/ali.$i.gz | $kaldisrc/bin/ali-to-pdf $targetdir/tri5a_ali/final.mdl ark:- ark,t:- >> $targetdir/pdfs
done
